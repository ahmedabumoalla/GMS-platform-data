...
import { 
    ArrowUpRight, Save, PlusCircle, Trash2, ArrowRight, ArrowLeft, 
    Calculator, AlertTriangle, CheckCircle2, FileText, Loader2, 
...
export default function PaymentVoucherPage() {
    ...
    const [form, setForm] = useState({
        ...
        reference: `PV-${Date.now().toString().slice(-5)}`,
        amount: '',
        treasury_id: '',
    ...
    // Lines for accounting distribution
    ...
    const handleSave = async () => {
        ...
        try {
            const { error } = await supabase.from('vouchers').insert({
                voucher_type: 'payment', reference_number: form.reference, date: form.date, amount: mainAmount,
            ...
            alert('تم إصدار سند الصرف بنجاح!');
    ...
    const inputBg = isDark ? "bg-slate-800 border-slate-700 text-white focus:border-rose-500" : "bg-slate-50 border-slate-200 text-slate-900 focus:border-rose-500 focus:bg-white";
                ...
                <div className="flex items-center gap-4 max-w-7xl mx-auto">
                    ...
                    <div>
                        <h1 className={`text-2xl font-black flex items-center gap-3 ${isDark ? 'text-white' : 'text-slate-900'}`}>
                            <div className="p-2 bg-rose-100 text-rose-600 rounded-xl"><ArrowUpRight size={24}/></div>
                            سند صرف نقدية (Payment Voucher)
                        </h1>
                        <p className={`text-sm mt-1 font-medium ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>إثبات خروج نقدية وتوجيهها للحسابات المعنية.</p>
                    </div>
                </div>
            </div>
            ...
            <div className="max-w-7xl mx-auto px-6 mt-8 space-y-6">
                ...
                <div className={`p-8 rounded-[2rem] border shadow-sm ${cardBg}`}>
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                        <div className="md:col-span-2">
                            <label className="text-xs font-bold text-slate-500 mb-2 block">المبلغ (SAR) *</label>
                            <input type="number" value={form.amount} onChange={e=>setForm({...form, amount: e.target.value})} placeholder="0.00" className={`w-full p-4 rounded-2xl outline-none font-mono font-black text-2xl text-rose-600 transition ${inputBg}`} />
                        </div>
                        <div>
                        ...
                        <div>
                            <label className="text-xs font-bold text-slate-500 mb-2 block">يُصرف من (الخزينة/البنك) *</label>
                        ...
                        <div>
                            <label className="text-xs font-bold text-slate-500 mb-2 block">طريقة الدفع</label>
                        ...
                        <div className="md:col-span-2">
                            <label className="text-xs font-bold text-slate-500 mb-2 block">يُصرف لأمر (المستفيد)</label>
                            <div className="relative">
                                <User className={`absolute top-4 text-slate-400 w-5 h-5 ${isRTL ? 'right-4' : 'left-4'}`} />
                                <input type="text" value={form.entity_name} onChange={e=>setForm({...form, entity_name: e.target.value})} placeholder="اسم المورد أو الموظف..." className={`w-full p-4 rounded-2xl outline-none font-bold text-sm transition ${isRTL ? 'pr-12 pl-4' : 'pl-12 pr-4'} ${inputBg}`} />
                            </div>
                        </div>
                        <div className="md:col-span-4">
                            <label className="text-xs font-bold text-slate-500 mb-2 block">البيان / الغرض من الصرف</label>
                            <input type="text" value={form.notes} onChange={e=>setForm({...form, notes: e.target.value})} placeholder="اكتب تفاصيل سبب الصرف هنا..." className={`w-full p-4 rounded-2xl outline-none font-medium text-sm transition ${inputBg}`} />
                        </div>
                    </div>
                </div>
                ...
                <div className={`rounded-[2rem] border shadow-sm overflow-hidden ${cardBg}`}>
                    <div className="p-6 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
                        <h2 className="text-sm font-black text-slate-700 dark:text-slate-300">التوجيه المحاسبي (الحسابات المدينة)</h2>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="w-full text-start">
                            <thead className={`text-xs uppercase font-black border-b ${isDark ? 'bg-slate-800/50 text-slate-400 border-slate-700' : 'bg-white text-slate-500 border-slate-200'}`}>
                                <tr>
                                    <th className={`p-4 w-1/3 ${isRTL ? 'text-right' : 'text-left'}`}>الحساب الموجه إليه</th>
                                        ...
                                        <motion.tr initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} key={line.id} className="group">
                                            <td className="p-3">
                                                <select value={line.account_id} onChange={e => updateLine(line.id, 'account_id', e.target.value)} className={`w-full p-3 rounded-xl border outline-none font-bold text-xs cursor-pointer ${inputBg}`}>
                                                    <option value="">-- اختر حساب المصروف/المورد --</option>
                                            ...
                                            <td className="p-3">
                                                <input type="number" value={line.amount} onChange={e => updateLine(line.id, 'amount', e.target.value)} placeholder="0.00" className={`w-full p-3 rounded-xl border outline-none font-mono font-black text-sm text-rose-500 focus:border-rose-500 focus:ring-1 focus:ring-rose-500 ${isRTL ? 'text-left' : 'text-right'} ${isDark ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-200'}`} />
                                            </td>
                                            <td className="p-3 text-center">
                                                <button onClick={() => removeLine(line.id)} disabled={lines.length <= 1} className="p-2 text-slate-300 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition disabled:opacity-30 cursor-pointer"><Trash2 size={18}/></button>
                                            </td>
                                        </motion.tr>
                                    ))}
                                </AnimatePresence>
                    ...
                    <div className={`p-4 border-t ${isDark ? 'border-slate-800 bg-slate-900/50' : 'border-slate-100 bg-slate-50'}`}>
                        <button onClick={addLine} className="flex items-center gap-2 text-sm font-bold text-rose-600 bg-rose-50 hover:bg-rose-100 px-4 py-2.5 rounded-xl transition">
                ...
                <div className="max-w-7xl mx-auto px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-6">
                    <div className="flex items-center gap-6 lg:gap-12 w-full md:w-auto">
                        <div className={isRTL ? 'text-left' : 'text-right'}>
                            ...
                            <div className="text-2xl font-mono font-black text-rose-600">{mainAmount.toLocaleString('en-US', { minimumFractionDigits: 2 })}</div>
                        </div>
