...
type Employee = {
  ...
  phone?: string;
  email?: string;
  project_performance: number; // جديد: الأداء في هذا المشروع
  overall_performance: number; // جديد: الأداء العام في الشركة
};
...
export default function ContractDetailsPage({ params }: PageProps) {
  ...
  const fetchContractDetails = async () => {
    ...
    setTimeout(() => {
        ...
        // بيانات الفريق الوهمية (بالعربي مع إحصائيات الأداء)
        setTeam([
            { 
  ...
  return (
    <div className="space-y-8 font-sans pb-10" dir="rtl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* القسم الأيمن: التفاصيل وعرض PDF */}
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                ...
                {contract.pdf_url ? (
                    ...
                    <div className="text-center py-10 bg-slate-50 rounded-2xl border border-slate-100 text-slate-400">
                        ...
                        لم يتم رفع ملف PDF لهذا العقد بعد.
                    </div>
                )}
            </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 h-fit">
            <h3 className="font-bold text-lg mb-4 flex items-center gap-2">
                <Users className="text-slate-400" /> الفريق الحالي
            </h3>
            <div className="space-y-4">
                {team.length === 0 ? (
                    <p className="text-slate-400 text-sm text-center py-4">لم يتم تعيين فريق بعد.</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[85vh]">
                ...
                <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                    <div>
                        <h3 className="font-bold text-lg text-slate-800">قائمة فريق العمل الكاملة</h3>
                            ...
                            {team.map((member) => (
                                <div key={member.id} className="bg-white p-5 rounded-xl border border-slate-200 hover:shadow-md transition group flex flex-col justify-between">
                                    ...
                                    <div className="flex justify-between items-start mb-4">
                                        ...
                                        <div className="text-[10px] text-slate-300 font-mono">ID: {member.id}</div>
                                    </div>
