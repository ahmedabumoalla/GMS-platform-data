...
import { 
  ...
  X, Plus, Loader2, ShieldCheck, Wallet, Edit2, ExternalLink,
  User // 👈 تم إضافة أيقونة User هنا
} from 'lucide-react';
...
export default function EditSubcontractorPage() {
      ...
      try {
        ...
        if (data) {
          setBasicInfo({
            ...
            cr_number: data.company_reg_number || '',
            tax_number: data.tax_number || '',
            national_address: data.national_address || '',
            bank_account: data.bank_account || '',
            payment_terms: data.payment_terms || '',
            account_type: data.account_type || 'Credit',
          ...
          if (data.broker_details && Object.keys(data.broker_details).length > 0) {
              setHasBroker(true);
              setBroker(data.broker_details);
          }

          if (data.petty_cash_details && Object.keys(data.petty_cash_details).length > 0) {
              setHasPettyCash(true);
              setPettyCash(data.petty_cash_details);
          }

          if (data.document_urls) {
              setExistingDocs(data.document_urls);
          }
        }
      } catch (error) {
 ...
 const uploadFileToSupabase = async (file: File | null, folder: string, existingUrl: string) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
          const document_urls = {
              cr_url: await uploadFileToSupabase(files.crFile, 'cr', existingDocs?.cr_url),
              tax_url: await uploadFileToSupabase(files.taxFile, 'tax', existingDocs?.tax_url),
              address_url: await uploadFileToSupabase(files.addressFile, 'address', existingDocs?.address_url),
              contract_url: await uploadFileToSupabase(files.contractFile, 'contracts', existingDocs?.contract_url),
              broker_id_url: hasBroker ? await uploadFileToSupabase(files.brokerIdFile, 'broker', existingDocs?.broker_id_url) : null
          };
          ...
          alert(isRTL ? 'تم تحديث بيانات المقاول بنجاح!' : 'Subcontractor updated successfully!');
          window.location.href = '/dashboard/subcontractors';

      } catch (error: any) {
          console.error(error);
          alert(isRTL ? 'حدث خطأ أثناء التحديث' : 'Error updating data');
      } finally {
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className={`text-2xl font-black flex items-center gap-3 ${textMain}`}>
                <Edit2 className="text-amber-500" /> {isRTL ? 'تعديل بيانات المقاول' : 'Edit Subcontractor'}
              </h1>
              <p className={`text-sm font-medium mt-1 ${textSub}`}>{basicInfo.name}</p>
            </div>
          </div>
      ...
      <main className="max-w-5xl mx-auto p-6 mt-4 space-y-8">
        ...
        <section className={`p-8 rounded-[2rem] shadow-sm ${glassCard}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <label className={`text-xs font-bold uppercase tracking-wider mb-2 block ${textSub}`}>{isRTL ? 'تاريخ بداية التعامل' : 'Deal Start Date'}</label>
                    <div className="relative">
        ...
        <section className={`p-8 rounded-[2rem] shadow-sm ${glassCard}`}>
            <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-100 dark:border-slate-800">
                <h3 className={`text-lg font-black flex items-center gap-2 ${textMain}`}>
                    <Users className="text-purple-500" size={20}/> {isRTL ? 'الأشخاص المسؤولون' : 'Contact Persons'}
                </h3>
                    ...
                    <div key={idx} className={`p-6 rounded-2xl border ${isDark ? 'bg-slate-800/30 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                ...
                                <input type="text" value={contact.name} onChange={(e) => handleContactChange(idx, 'name', e.target.value)} className={`w-full p-3 rounded-xl border text-sm font-bold ${inputBg}`} />
                            </div>
                            <div>
                                <label className={`text-[10px] font-bold uppercase mb-1 block ${textSub}`}>{isRTL ? 'الإيميل' : 'Email'}</label>
                                <input type="email" dir="ltr" value={contact.email} onChange={(e) => handleContactChange(idx, 'email', e.target.value)} className={`w-full p-3 rounded-xl border text-sm font-bold rtl:text-right ${inputBg}`} />
                            </div>
                            <div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <input type="text" dir="ltr" value={broker.percentage} onChange={e=>setBroker({...broker, percentage:e.target.value})} className={`w-full p-3 rounded-xl border text-sm font-bold rtl:pr-9 ltr:pl-9 rtl:text-right ${inputBg}`} />
                                    </div>
                                </div>
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                        ...
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="space-y-4 overflow-hidden">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="number" value={pettyCash.amount} onChange={e=>setPettyCash({...pettyCash, amount:e.target.value})} className={`w-full p-3 rounded-xl border text-sm font-bold ${inputBg}`} />
                                </div>
                                <div>
                            ...
                            <div>
                                ...
                                <textarea rows={2} value={pettyCash.details} onChange={e=>setPettyCash({...pettyCash, details:e.target.value})} className={`w-full p-3 rounded-xl border text-sm font-bold resize-none ${inputBg}`} />
                            </div>
                        </motion.div>
                    )}
                </AnimatePresence>
        ...
        <section className={`p-8 rounded-[2rem] shadow-sm ${glassCard}`}>
            <h3 className={`text-lg font-black flex items-center gap-2 mb-6 pb-4 border-b ${isDark ? 'border-slate-800 text-white' : 'border-slate-100 text-slate-900'}`}>
                <UploadCloud className="text-indigo-500" size={20}/> {isRTL ? 'تحديث المرفقات الرسمية' : 'Update Documents'}
            </h3>
            
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {/* 👈 تم إضافة النوع (e: any) لمنع أخطاء الـ TypeScript الصارمة */}
                <FileUploadBox label={isRTL ? 'صورة السجل التجاري' : 'CR Copy'} file={files.crFile} existingUrl={existingDocs?.cr_url} onChange={(e: any) => handleFileChange(e, 'crFile')} isDark={isDark} />
                <FileUploadBox label={isRTL ? 'الشهادة الضريبية' : 'Tax Certificate'} file={files.taxFile} existingUrl={existingDocs?.tax_url} onChange={(e: any) => handleFileChange(e, 'taxFile')} isDark={isDark} />
                <FileUploadBox label={isRTL ? 'العنوان الوطني' : 'National Address'} file={files.addressFile} existingUrl={existingDocs?.address_url} onChange={(e: any) => handleFileChange(e, 'addressFile')} isDark={isDark} />
                <FileUploadBox label={isRTL ? 'نسخة العقد المبرم' : 'Contract Copy'} file={files.contractFile} existingUrl={existingDocs?.contract_url} onChange={(e: any) => handleFileChange(e, 'contractFile')} isDark={isDark} />
                {hasBroker && (
                    <FileUploadBox label={isRTL ? 'هوية الوسيط' : 'Broker ID'} file={files.brokerIdFile} existingUrl={existingDocs?.broker_id_url} onChange={(e: any) => handleFileChange(e, 'brokerIdFile')} isDark={isDark} />
                )}
            </div>
        </section>
...
function FileUploadBox({ label, file, existingUrl, onChange, isDark }: { label: string, file: File | null, existingUrl?: string, onChange: any, isDark: boolean }) {
    return (
        <div className={`border-2 border-dashed rounded-2xl p-4 text-center relative overflow-hidden transition-all ${file ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20' : existingUrl ? 'border-blue-300 bg-blue-50/50 dark:border-blue-800 dark:bg-blue-900/10' : isDark ? 'border-slate-700 bg-slate-800/50 hover:border-blue-500' : 'border-slate-300 bg-slate-50 hover:border-blue-400'}`}>
            ...
            <div className="flex flex-col items-center justify-center gap-2 relative z-0">
                {file ? <CheckCircle2 className="text-emerald-500" size={24} /> : <FileText className={existingUrl ? "text-blue-500" : "text-slate-400"} size={24} />}
                <div className={`text-xs font-bold ${file ? 'text-emerald-700 dark:text-emerald-400' : existingUrl ? 'text-blue-700 dark:text-blue-400' : isDark ? 'text-white' : 'text-slate-700'}`}>{label}</div>
                
                {file ? (
                    <div className="text-[10px] text-emerald-600 truncate max-w-[150px]">{file.name}</div>
                ) : existingUrl ? (
                    <div className="flex items-center gap-1 text-[10px] text-blue-500 font-bold bg-blue-100 px-2 py-0.5 rounded-full z-20 pointer-events-auto">
                        <ExternalLink size={10}/> <a href={existingUrl} target="_blank" rel="noopener noreferrer" className="hover:underline">عرض الملف الحالي</a>
                    </div>
                ) : (
                    <div className="text-[10px] text-slate-500">لم يتم الرفع</div>
                )}
            </div>
        </div>
    );
